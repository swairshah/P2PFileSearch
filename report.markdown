...
#### FileTransfer
`FileTransfer` facilitates transferring of requested file and its corresponding
metadata to the client node with the help of `FileServer`. Using the client 
socket information received by `FileServer`, it retrieves the `filename` 
passed by `Downloader` to itself. 
